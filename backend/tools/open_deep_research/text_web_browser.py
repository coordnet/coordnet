...
class SimpleTextBrowser:
    """
    (In preview) An extremely simple text-based web browser comparable to Lynx. Suitable for
    Agentic use.
    """
    ...
    def find_on_page(self, query: str) -> str | None:
        """
        Searches for the query from the current viewport forward, looping back to the start if
        necessary.
        """
    ...
    def _split_pages(self) -> None:
        ...
        while start_idx < len(self._page_content):
            ...
            while end_idx < len(self._page_content) and self._page_content[end_idx - 1] not in [
                " ",
                "\t",
                "\r",
                "\n",
        ...
        def _prev_visit(url):
            for i in range(len(self.history) - 1, -1, -1):
                if self.history[i][0] == url:
                    return (
                        "You previously visited this page "
                        f"{round(time.time() - self.history[i][1])} seconds ago.\n"
                    )
            return ""
    ...
    def _fetch_page(self, url: str) -> None:
        ...
        try:
            if url.startswith("file://"):
                ...
                request_kwargs = (
                    self.request_kwargs.copy() if self.request_kwargs is not None else {}
                )
