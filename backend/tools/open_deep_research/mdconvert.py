...
class YouTubeConverter(DocumentConverter):
    ...
    def convert(self, local_path: str, **kwargs: Any) -> None | DocumentConverterResult:
        ...
        # We can also try to read the full description. This is more prone to breaking, since it
        # reaches into the page implementation
        try:
...
class PdfConverter(DocumentConverter):
    """
    Converts PDFs to Markdown. Most style information is ignored, so the results are essentially
    plain-text.
    """
...
class DocxConverter(HtmlConverter):
    """
    Converts DOCX files to Markdown. Style information (e.g.m headings) and tables are preserved
    where possible.
    """
        ...
        if not exiftool:
            ...
            try:
                result = subprocess.run(
                    [exiftool, "-json", local_path], capture_output=True, text=True, check=False
                ).stdout
...
class WavConverter(MediaConverter):
    """
    Converts WAV files to markdown via extraction of metadata (if `exiftool` is installed), and
    speech transcription (if `speech_recognition` is installed).
    """
...
class Mp3Converter(WavConverter):
    """
    Converts MP3 and M4A files to markdown via extraction of metadata (if `exiftool` is installed),
    and speech transcription (if `speech_recognition` AND `pydub` are installed).
    """
    ...
    def convert(self, local_path, **kwargs) -> None | DocumentConverterResult:
        ...
        try:
            ...
            _args = {**kwargs, "file_extension": ".wav"}
