...
export const addToCanvas = async (server: Hocuspocus, req: Request, res: Response) => {
  const result = requestSchema.safeParse(req.body);
  if (!result.success) {
    return res.status(400).json({ errors: result.error.flatten() });
  }

  const { spaceId, canvasId, nodes: incomingNodes, edges: incomingEdges } = result.data;

  const docConnection = await server.openDirectConnection(`node-graph-${canvasId}`, {});
