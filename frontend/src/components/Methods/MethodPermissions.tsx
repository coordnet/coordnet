...
const MethodPermissions = ({
  ...
  const { data: permissions, isLoading: permissionsLoading } = useQuery({
    queryKey: ["methods", id, "permissions"],
    queryFn: ({ signal }) => getPermissions(signal, PermissionModel.Method, id),
    enabled: Boolean(id),
    initialData: [],
  });

  if (!parent?.data || parent.type != BackendEntityType.METHOD) return null;
  const method = parent.data;

  if (!id || !method || permissionsLoading || userLoading)
    return (
  ...
  const icon = blockies.create({ seed: method?.id }).toDataURL();
  ...
  return (
    <div className={clsx("flex flex-col gap-8 p-6", className)}>
      <div className="flex">
        ...
        <div>
          <div className="mb-2 text-lg font-semibold">{method?.title}</div>
            ...
            {ownPermissions?.role?.toLowerCase() == "owner" && (
              <Dialog open={memberOpen} onOpenChange={setMemberOpen}>
                ...
                <DialogContent className="w-[400px] p-0">
                  <Member method={method} setOpen={setMemberOpen} />
                </DialogContent>
              </Dialog>
            )}
          </div>

          <PermissionsList method={method} />
        </div>
      )}
...
export default MethodPermissions;
