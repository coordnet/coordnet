...
import { CustomError, getHocuspocusYdoc } from "@/lib/utils";
...
export const NodesContextProvider = ({
  ...
  const syncDisconnectedDocument = useCallback(async () => {
    try {
      await getHocuspocusYdoc(name, document!, token);
      setSynced(true);
      setConnected(true);
    } catch (error) {
  ...
  useEffect(() => {
    if ((!spaceId && !methodId) || !document) return;
    if (isMethodRun) {
      syncDisconnectedDocument();
      return;
    }
  ...
  const value = {
    ...
    breadcrumbs,
    setBreadcrumbs,
    scope: isMethodRun ? "read-write" : provider?.authorizedScope,
    isMethodRun,
  };
